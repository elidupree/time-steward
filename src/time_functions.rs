...
fn updated_by (poly: & Polynomial <Coordinate> , time: Coordinate, time_scale_shift: i32)->Polynomial <Coordinate>  {
  &*poly + Polynomial::new (vec![((poly.data().get (1).cloned().unwrap_or (0)*time) >> time_scale_shift) + ((poly.data().get (2).cloned().unwrap_or (0)*time*time) >> (time_scale_shift*2)), ((poly.data().get (2).cloned().unwrap_or (0)*time*2) >> time_scale_shift), 0i64])
  } 
impl QuadraticTrajectory {
  ...
  pub fn updated_by (& self, time: Coordinate)->QuadraticTrajectory {
    QuadraticTrajectory {
    data: Vector2::new (
    updated_by (& self.data [0], time, self.time_scale_shift),
    updated_by (& self.data [1], time, self.time_scale_shift)),..self.clone()}
  }
  ...
  Vector2::new (self.data [0].data().get (0).cloned().unwrap_or (0),self.data [1].data().get (0).cloned().unwrap_or (0))
...
if result >0 {
let checker = Vector2::new (updated_by (& displacement_function [0], result,first.1.time_scale_shift), updated_by (& displacement_function [1], result,first.1.time_scale_shift));
