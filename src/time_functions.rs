...
fn distance_squared_would_be ( first: (Coordinate, &QuadraticTrajectory),
                                            second: (Coordinate, &QuadraticTrajectory), when: Coordinate)->Coordinate
{
let displacement = first.1.updated_by (when - first.0).evaluate() - second.1.updated_by (when - second.0).evaluate();
...
let mut result = self.clone(); result.update_by (time); result
      }

  pub fn update_by(&mut self, time: Coordinate) {
    for quadratic in self.data.iter_mut() {quadratic_move_origin_rounding_change_towards_0 (quadratic.as_mut(), time, self.time_scale_shift);}
  }
  ...
  pub fn approximately_when_distance_passes(distance: Coordinate,
    ...
    if (distance_squared_would_be (first, second, base) - distance*distance)*direction >0 {return Some(base);}
