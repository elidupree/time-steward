...
struct Detector <B: Basics> {_marker: PhantomData <B>,}
impl <B: Basics> Column for Detector <B> {
type FieldType = HashSet <RowId>;
fn column_id()->ColumnId {ColumnId (B::nearness_column_id().0 ^ 0xdaa7e18546759b65)}
}


pub fn insert <B: Basics, M: Mutator <B::StewardBasics>> (mutator: &mut M, who: RowId, me: B::DetectorId) {
...
pub fn erase <B: Basics, M: Mutator <B::StewardBasics>> (mutator: &mut M, who: RowId, me: B::DetectorId) {
let my_row =RowId::new (& me);
let mut members = mutator.get ::<Detector <B>> (my_row).unwrap().clone();
members.remove (& who);
for member in members.iter() {
