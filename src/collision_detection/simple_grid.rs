...
pub trait Basics: super::Basics {
  fn get_bounds <A: MomentaryAccessor <<Self as super::Basics>::StewardBasics>>(accessor: & A, who: RowId)->Bounds;
...
fn cell_row <DetectorId: Hash> (x: i64, y: i64, me: DetectorId)->RowId {RowId::new (& (x, y, me))}
fn get_bounds <B: Basics, M: Mutator<B::StewardBasics>>(mutator: &mut M,
                                                       who: RowId,
                                                       me: B::DetectorId)->Bounds {
...
pub fn insert<B: Basics, M: Mutator<B::StewardBasics>>(mutator: &mut M,
  ...
  mutator.set::<Member <B>> (RowId::new (& (who, me)), Some (Member {row: who, bounds: bounds, detector: me,_marker: PhantomData}));
  for next in bounds.min [0].. bounds.max [0] + 1 {for what in bounds.min [1].. bounds.max [1] + 1 {
