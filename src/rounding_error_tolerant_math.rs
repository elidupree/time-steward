use std::ops::{Add, Sub, Mul, Div, Neg, Shr};
use std::cmp::{max, min};
...
(A: Range `operator` B: Range) yields a Range that is a (non-strict) superset of (for all a in A and all b in B, a `operator` b). It prefers to yield as SMALL a possible set as it can, but doesn't guarantee that. For addition, subtraction, and multiplication, this is a generalization of i64 math. For division, it is slightly different (effectively, it rounds in BOTH directions).

Range is also a partially floating-point type: it handles overflow by increasing an exponent value and rounding off. In keeping with its strict behavior, this rounding can only INCREASE the size of the Range, not decrease it.
...
//TODO: mess with exponent 
if result.min <0 {result.min = (result.min + 1 - other.min)/other.min;}
 else {result.min = (result.min + 1 - other.max)/other.max;}
 if result.max <0 {result.max = (result.max)/other.max;}
  else {result.min = (result.max)/other.min;}
result
}
}
...
Returns a collection of ranges that include the exact roots.
...
#[test]
fn tests() {
printlnerr!(" {:?}", roots_quadratic ([Range::new (- 900, - 800), Range::new (500, 501), Range::exactly (50)]))
}
