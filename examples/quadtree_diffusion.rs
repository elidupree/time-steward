...
fn ideal_transfer_velocity (nodes: [& ?????;2]) {
  let density_difference = ink_at(nodes[0], time)/(nodes [0].width*nodes [0].width) - ink_at(nodes[1], time)/(nodes [1].width*nodes [1].width);
...
fn update_transfer_change_prediction (nodes: [& ?????;2], boundary:?) {
  let trajectories = [
    inferred_density_trajectory_at (nodes [0], boundary.center, *accessor.now()),
    inferred_density_trajectory_at (nodes [1], boundary.center, *accessor.now()),
  ];
  let difference = [
  ...
  let permissible_error = difference [1].abs()/8 + 2;
  let time = if difference [0].abs() >= permissible_error {
    Some(*accessor.now())
  } else {
    Some(
  }
