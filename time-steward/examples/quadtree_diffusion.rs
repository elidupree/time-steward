...
impl TreeContinuumPhysics for Physics {
  ...
  fn initialize_split_boundary<A: EventAccessor<Steward = Self::Steward>>(
    ...
    BoundaryVarying {
      transfer_velocity: match boundary.old_boundary {
        None => 0,
        Some(previous) => {
          query_ref(accessor, &previous.varying)
            .data
            .transfer_velocity
        }
      },
...
fn update_inferred_node_properties<A: EventAccessor<Steward = Steward>>(
  accessor: &A,
  node: &NodeHandle,
) {
...

  {
    ...
    iterate_boundaries(&varying.boundaries, |dimension, direction, boundary| {
      ...
      let transfer_rate = query_ref(accessor, &boundary.varying)
        .data
        .transfer_velocity
        * boundary.length as Amount;
...
fn ink_at<A: Accessor<Steward = Steward>>(accessor: &A, node: &NodeHandle, time: Time) -> Amount {
  ...
  varying.data.ink_at_last_change
    + varying.data.accumulation_rate * ((time - varying.data.last_change) as Amount)
}
...
fn density_difference_to_ideal_transfer_velocity(
  nodes: [&NodeHandle; 2],
  density_difference: Amount,
) -> Amount {
...
fn transfer_velocity_to_ideal_density_difference(
  nodes: [&NodeHandle; 2],
  transfer_velocity: Amount,
) -> Amount {
...
fn update_transfer_change_prediction<A: EventAccessor<Steward = Steward>>(
  ...
  set_with!(accessor, &boundary.varying, |boundary_varying| {
    ...
    let accumulation_rates = [
      unwrap_leaf_ref(&query_ref(accessor, &nodes[0].varying))
        .data
        .accumulation_rate,
      unwrap_leaf_ref(&query_ref(accessor, &nodes[1].varying))
        .data
        .accumulation_rate,
    ];
    ...
    let density_accumulation_difference_scaled =
      density_accumulations_scaled[0] - density_accumulations_scaled[1];
    let numerator = accumulation_rates[0] * areas[1] - accumulation_rates[1] * areas[0];
    let denominator =
      4 * (nodes[0].width as Amount * areas[1] + nodes[1].width as Amount * areas[0]);
    let max_reasonable_velocity_skew = numerator / denominator;

    // the more unstable the boundary is, the more error we permit
    let max_rounding_error = max(
      VELOCITY_PER_SLOPE,
      (nodes[0].width + nodes[1].width) as Amount,
    );
    let permissible_error = 2 * max_rounding_error
      + max_reasonable_velocity_skew.abs()
      + GENERIC_INK_AMOUNT / (SECOND as Amount) / (METER as Amount) / 1000000; // + density_accumulation_difference.abs()*boundary.length as Amount/8;
    ...
    let (min_difference, max_difference) = (
      transfer_velocity_to_ideal_density_difference(
        [&nodes[0], &nodes[1]],
        boundary_varying.data.transfer_velocity - permissible_error,
      ),
      transfer_velocity_to_ideal_density_difference(
        [&nodes[0], &nodes[1]],
        boundary_varying.data.transfer_velocity + permissible_error,
    ...
    let time = if difference_now < min_difference || difference_now > max_difference {
      ...
      Some(
        *accessor.now()
          + safer_scaling(
            max_difference - difference_now,
            scale_factor,
            density_accumulation_difference_scaled,
          ) as Time,
      )
    } else if density_accumulation_difference_scaled < 0 {
      Some(
    ...
    boundary_varying.data.next_change = time.map(|time| {
      accessor.create_prediction(
        time,
        EntityId::hash_of(&(accessor.id(), boundary.center)),
        TransferChange {
...
fn maybe_split<A: EventAccessor<Steward = Steward>>(accessor: &A, node: &NodeHandle) -> bool {
  ...
  let mut close_enough = true;
  {
    ...
    iterate_boundaries(&varying.boundaries, |dimension, _direction, boundary| {
      let density_difference = density_at(accessor, &boundary.nodes[0], *accessor.now())
        - density_at(accessor, &boundary.nodes[1], *accessor.now());
      let ideal_velocity = density_difference_to_ideal_transfer_velocity(
        [&boundary.nodes[0], &boundary.nodes[0]],
        density_difference,
      );
...
fn maybe_merge<A: EventAccessor<Steward = Steward>>(accessor: &A, node: &NodeHandle) -> bool {
  if just_destroyed(accessor, &node.varying) {
    return false;
  }
  let mut close_enough = true;
  {
    ...
    iterate_children(&varying.children, |_coordinates, child| {
      ...
      let child_varying = match *child_guard {
        tree_continuum::NodeVarying::Branch(_) => {
          abort = true;
          return;
        }
    ...
    iterate_children(&varying.children, |_coordinates, child| {
      ...
      iterate_boundaries(
        &child_varying.boundaries,
        |dimension, _direction, boundary| {
          let density_difference = density_at(accessor, &boundary.nodes[0], *accessor.now())
            - density_at(accessor, &boundary.nodes[1], *accessor.now());
          let ideal_velocity = density_difference_to_ideal_transfer_velocity(
            [&boundary.nodes[0], &boundary.nodes[0]],
...
fn merge<A: EventAccessor<Steward = Steward>>(accessor: &A, node: &NodeHandle) {
...

  {
    ...
    iterate_boundaries(
      &leaf_varying.boundaries,
      |_dimension, _direction, boundary| {
        update_transfer_change_prediction(accessor, boundary);
      },
    );
  }
...
impl Event for TransferChange {
  ...
  fn execute<Accessor: EventAccessor<Steward = Self::Steward>>(&self, accessor: &mut Accessor) {
    ...
    let difference_now = densities[0] - densities[1];
    let physics_ideal_velocity =
      density_difference_to_ideal_transfer_velocity([&nodes[0], &nodes[1]], difference_now);
    set_with!(accessor, &self.boundary.varying, |boundary_varying| {
      ...
      accumulation_rates[0] -= (physics_ideal_velocity - boundary_varying.data.transfer_velocity)
        * self.boundary.length as Amount;
      accumulation_rates[1] += (physics_ideal_velocity - boundary_varying.data.transfer_velocity)
        * self.boundary.length as Amount;
      ...
      assert!(
        (accumulation_rates[0] * areas[1] - accumulation_rates[1] * areas[0]).abs()
          >= ((accumulation_rates[0]
            - max_reasonable_velocity_skew * self.boundary.length as Amount)
            * areas[1]
            - (accumulation_rates[1]
              + max_reasonable_velocity_skew * self.boundary.length as Amount)
              * areas[0])
            .abs()
      );
      //printlnerr!("{:?}", max_reasonable_velocity_skew);
      boundary_varying.data.transfer_velocity =
        physics_ideal_velocity + numerator.signum() + max_reasonable_velocity_skew / 2;
    });
    for node in nodes.iter() {
      update_inferred_node_properties(accessor, node);
    }
    for node in nodes.iter() {
      let guard = query_ref(accessor, &node.varying);
      iterate_boundaries(
        &unwrap_leaf_ref(&guard).boundaries,
        |_dimension, _direction, boundary| {
          update_transfer_change_prediction(accessor, boundary);
        },
      );
    ...
    let mut split = false;
    for node in nodes.iter() {
      if match *query_ref(accessor, &node.varying) {
        tree_continuum::NodeVarying::Leaf(_) => true,
        _ => false,
      } {
        //audit(accessor);
        if maybe_split(accessor, node) {
          split = true;
        }
        //audit(accessor);
      }
      ...
      match query(accessor, &node.varying) {
        tree_continuum::NodeVarying::Branch(ref b) => {
          node = child_by_coordinates(
            &b.children,
            [
              if self.coordinates[0] > node.center[0] {
                1
              } else {
                0
              },
              if self.coordinates[1] > node.center[1] {
                1
              } else {
                0
              },
      ...
      match ev {
        ...
        glium::glutin::Event::MouseMoved(x, y) => {
          let display_size = display
            ...
            .unwrap();
          mouse_coordinates[0] = (x as Distance) * accessor.globals().size[0]
            / display_size.0 as Distance
            - accessor.globals().size[0] / 2;
          mouse_coordinates[1] = (display_size.1 as Distance - (y as Distance))
            * accessor.globals().size[1]
            / display_size.1 as Distance
            - accessor.globals().size[1] / 2;
        }
            ...
            .insert_fiat_event(
              ...
              AddInk {
                ...
                amount: ((input_derivative + 1) % 2)
                  * (GENERIC_INK_AMOUNT << input_magnitude_shift)
                  * input_signum,
      ...
      stew
        .insert_fiat_event(
          ...
          AddInk {
            coordinates: [mouse_coordinates[0], mouse_coordinates[1]],
            amount: ((input_derivative + 1) % 2)
              * (GENERIC_INK_AMOUNT * 8 << input_magnitude_shift)
              * input_signum,
            accumulation: input_derivative
              * (GENERIC_INK_AMOUNT * 8 << input_magnitude_shift)
              * input_signum
        ...
        tree_continuum::NodeVarying::Leaf(ref leaf) => {
          ...
          let (my_current_ink, slope) = match display_state {
            0 => (
              ...
              [
                leaf.data.slope[0] as f32 * size_factor / density_factor,
                leaf.data.slope[1] as f32 * size_factor / density_factor,
              ],
            ),
            1 => (
              ((leaf.data.accumulation_rate * (SECOND as Amount) * 100
                / (handle.width * handle.width) as Amount) as f32)
                / density_factor,
              [0.0, 0.0],
            ),
            _ => (
              (accessor.now() - leaf.data.last_change) as f32 / SECOND as f32,
              [0.0, 0.0],
            ),
          };
