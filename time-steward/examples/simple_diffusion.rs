...
use simple_timeline::{query, set, unset, SimpleTimeline};
use std::cmp::{max, min};
use steward_module::{
  simple_timeline, Accessor, ConstructibleTimeSteward, EntityCell, Event, EventAccessor,
  EventHandle, FutureCleanupAccessor, IncrementalTimeSteward, TimeSteward,
};
...
fn more_stable_rate_and_cumulative_error_coefficients(
  cells: [&CellVarying; 2],
  accumulation_rates: [i64; 2],
  transfer: &TransferVarying,
  start: Time,
) -> (i64, (i64, i64)) {
  ...
  (
    rate_change_stability_thing,
    (rate_change_stability_thing, 0),
  )
}

fn desired_transfer_change_time(
  cells: [&CellVarying; 2],
  accumulation_rates: [i64; 2],
  transfer: &TransferVarying,
  start: Time,
) -> Option<Time> {
  let (rate_change_stability_thing, coefficients) =
    more_stable_rate_and_cumulative_error_coefficients(cells, accumulation_rates, transfer, start);

  let original_difference =
    ink_at(cells[0], accumulation_rates[0], start) - ink_at(cells[1], accumulation_rates[1], start);
  let difference_change_rate = accumulation_rates[0] - accumulation_rates[1];
  ...
  if false && !(difference_change_rate == 0) {
    ...
    loop {
      ...
      let later_difference = ink_at(cells[0], accumulation_rates[0], start + duration)
        - ink_at(cells[1], accumulation_rates[1], start + duration);
      let minimum_difference = if original_difference.signum() != later_difference.signum() {
        0
      } else {
        min(original_difference.abs(), later_difference.abs())
      };
      ...
      let error = transfer.accumulated_error
        + duration * duration * coefficients.1
        + duration * coefficients.0;
      //printlnerr!("{:?}", (error ));
      assert!(duration > 0);
      if error.abs() > permissible_cumulative_error {
        break;
      }
  ...
  if difference_change_rate > 0 {
    previous_duration = min(
      previous_duration,
      (max_difference - original_difference) / difference_change_rate,
    );
  } else if difference_change_rate < 0 {
    previous_duration = min(
      previous_duration,
      (min_difference - original_difference) / difference_change_rate,
    );
  }

  if previous_duration == i64::max_value() {
    return None;
  }
...
fn update_cell<A: EventAccessor<Steward = Steward>>(accessor: &A, coordinates: [i32; 2]) {
  ...
  let mut my_varying = query(accessor, &me.varying);
  ...
  set(accessor, &me.varying, my_varying);
}

fn update_accumulated_error<A: EventAccessor<Steward = Steward>>(
  accessor: &A,
  coordinates: [i32; 2],
  dimension: usize,
) {
  ...
  if neighbor_coordinates[dimension] >= accessor.globals().size[dimension] {
    return;
  }
  ...
  let varying = [
    query(accessor, &cells[0].varying),
    query(accessor, &cells[1].varying),
  ];
  ...
  let mut transfer = query(accessor, &cells[0].transfers[dimension]);

  let start = max(
    transfer.last_change,
    max(varying[0].last_change, varying[1].last_change),
  );
  let (_, coefficients) = more_stable_rate_and_cumulative_error_coefficients(
    [&varying[0], &varying[1]],
    accumulation_rates,
    &transfer,
    start,
  ...
  set(accessor, &cells[0].transfers[dimension], transfer);
}

fn update_transfer_change_prediction<A: EventAccessor<Steward = Steward>>(
  accessor: &A,
  coordinates: [i32; 2],
  dimension: usize,
) {
  ...
  let time = desired_transfer_change_time(
    [&varying[0], &varying[1]],
    accumulation_rates,
    &transfer,
    *accessor.now(),
  ...
  */
  ...
  transfer.next_change = time.map(|time| {
    accessor.create_prediction(
      time,
      EntityId::hash_of(&(accessor.id(), coordinates, dimension)),
      TransferChange {
...
fn get_accumulation_rate<A: Accessor<Steward = Steward>>(
  accessor: &A,
  coordinates: [i32; 2],
) -> i64 {
  let me = get_cell(accessor, coordinates).unwrap();
  let mut result = query(accessor, &me.varying).fiat_accumulation_rate;
  for dimension in 0..2 {
    result -= query(accessor, &me.transfers[dimension]).rate;
    ...
    if let Some(neighbor) = get_cell(accessor, neighbor_coordinates) {
      result += query(accessor, &neighbor.transfers[dimension]).rate;
    }
  }
  result
}
...
fn in_bounds(globals: &Globals, coordinates: [i32; 2]) -> bool {
  coordinates[0] >= 0
    && coordinates[0] < globals.size[0]
    && coordinates[1] >= 0
    && coordinates[1] < globals.size[1]
}
...
impl Event for TransferChange {
  ...
  fn execute<Accessor: EventAccessor<Steward = Self::Steward>>(&self, accessor: &mut Accessor) {
    ...
    let my_varying = query(accessor, &me.varying);
    let neighbor = get_cell(accessor, neighbor_coordinates).unwrap();
    let neighbor_varying = query(accessor, &neighbor.varying);
    let mut transfer = query(accessor, &me.transfers[self.dimension]);
    ...
    let physics_transfer_rate =
      (my_varying.ink_at_last_change - neighbor_varying.ink_at_last_change) / (2 * SECOND);
    ...
    let stability_adjustment =
      difference_change_rate.signum() * (difference_change_rate.abs() / 8 + 2);
    ...
    set(accessor, &me.transfers[self.dimension], transfer);
...
fn main() {
  ...
  /*if !cfg!(target_os = "emscripten") {
    ...
    if arguments.flag_listen {
      ...
      let mut steward: simply_synchronized::Steward<SimulationSpec, DefaultSteward <SimulationSpec>> = simply_synchronized::Steward::new(EntityId::hash_of (& 0u32), 0, SECOND>>3, constants, BufReader::new (stream.try_clone().unwrap()), BufWriter::new (stream));
      ...
      let steward: simply_synchronized::Steward<SimulationSpec, DefaultSteward <SimulationSpec>> = simply_synchronized::Steward::new(EntityId::hash_of (& 1u32), 0, SECOND>>3, constants, BufReader::new (stream.try_clone().unwrap()), BufWriter::new (stream));
      run (steward, |a,b| (a.settle_before (b)));
      return;
    }
  }*/
    ...
    for ev in display.poll_events() {
      match ev {
        ...
        glium::glutin::Event::MouseMoved(x, y) => {
          mouse_coordinates[0] = (x as i32) * 60
            / display
              .get_window()
              .unwrap()
              ...
              .insert_fiat_event(
                ...
                AddInk {
                  ...
                  amount: ((input_derivative + 1) % 2)
                    * (generic_amount << input_magnitude_shift)
                    * input_signum,
    ...
    for x in 0..globals.size[0] {
      for y in 0..globals.size[1] {
        ...
        let my_current_ink = match display_state {
          0 => ink_at(
            &my_varying,
            get_accumulation_rate(&accessor, [x, y]),
            *accessor.now(),
          ) as f32,
